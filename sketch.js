...
function preload() {
  ...
  volI = loadImage("Images/OtherStone/vol0.png")
  muteI = loadImage("Images/OtherStone/mute.png")
  dragI = loadImage("Images/OtherStone/drag0.png")
...
function setup() {
  ...
  pause.scale = main*0.075*1.2

  vol = createSprite(550*main, 700*main, 10, 10)
  vol.addImage(volI)
  vol.scale = 0.03*main

  vol2 = createSprite(550*main, 700*main, 10, 10)
  vol2.addImage(muteI)
  vol2.scale = 0.022*main
  vol2.visible = false

  bar = createSprite(670*main,700*main,150*main,7.5*main)
  bar.shapeColor = "black"

  bar1 = createSprite(670*main,700*main,150*main,30*main)
  bar1.shapeColor = "black"
  bar1.visible = false

  drag = createSprite(670*main,700*main,150*main,7.5*main)
  drag.addImage(dragI)
  drag.scale = 0.075*main
...
function draw() {
  ...
  fill("#000000")
   ...
   // console.log("Please Do Not Use The Console Window To Cheat")
  ...
  if(mouseIsOver(pause)){
    pause.scale = main*0.075*1.4
  } else {
    pause.scale = main*0.075*1.2
  }

  if(mouseIsOver(caveStoneB)){
    caveStoneB.scale = main*0.8*1.3
  } else {
    caveStoneB.scale = main*0.8
  }

  if(mouseIsOver(coveStoneB)){
    coveStoneB.scale = main*0.8*1.3
  } else {
    coveStoneB.scale = main*0.8
  }

  if(mouseIsOver(cloveStoneB)){
    cloveStoneB.scale = main*0.8*1.3
  } else {
    cloveStoneB.scale = main*0.8
  }

  if(mouseIsOver(cliveStoneB)){
    cliveStoneB.scale = main*1.3
  } else {
    cliveStoneB.scale = main
  }

  if(mouseIsOver(vol)){
    vol.scale = main*0.03*1.3
  } else {
    vol.scale = main*0.03
  }
  ...
  volumeA = (drag.x - (bar.x - (bar.width/2))) / bar.width
  
  if (volumeA < 0.05 ){
    volume = 0
  }
  
  if (volumeA > 0.05 && volumeA < 0.1){
    volume = 0.1
  }

  if (volumeA > 0.11 && volumeA < 0.2){
    volume = 0.2
  }

  if (volumeA > 0.2 && volumeA < 0.3){
    volume = 0.3
  }

  if (volumeA > 0.3 && volumeA < 0.4){
    volume = 0.4
  }

  if (volumeA > 0.4 && volumeA < 0.5){
    volume = 0.5
  }

  if (volumeA > 0.5 && volumeA < 0.6){
    volume = 0.6
  }

  if (volumeA > 0.6 && volumeA < 0.7){
    volume = 0.7
  }

  if (volumeA > 0.7 && volumeA < 0.8){
    volume = 0.8
  }

  if (volumeA > 0.8 && volumeA < 0.9){
    volume = 0.9
  }
  ...
  if (gameState === "pause") {
    ...
    if(mousePressedOver(bar1)){
      drag.x = World.mouseX
      drag.scale = 0.075*main*1.2
    } else {
    ...
    if(mousePressedOver(vol)){
      volume = 0
      drag.x = (bar.x - bar.width/2)
    }
